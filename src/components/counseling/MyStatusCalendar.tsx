...
import { List, ListItem, ListItemText, Paper, Grid, Pagination, Typography } from '@mui/material';
...
const MyStatusCalendar: React.FC<MyStatusCalendarProps> = ({ userNo, userType }) => {
  ...
  const fetchCounselings = async (page: number) => {
    try {
      const response = await axios.get<CounselingResponse>(`/api/counselings/${userType}/${userNo}?page=${page}&size=5`);
  ...
  const calendarEvents = useMemo( ()=> counselings.map(counseling => ({
    id: counseling.reqNo.toString(),
    calendarId: '1',
    title: userType === 'counselor' ? `${counseling.student.user.userName}` : `${counseling.department.deptName} 상담`,
    category: 'time',
          ...
          <Paper elevation={3} className="list-container">
            ...
            <List>
              {counselings.map((counseling) => (
                <ListItem key={counseling.reqNo} button onClick={() => handleSelectCounseling({ data: counseling })} >
                  <ListItemText
