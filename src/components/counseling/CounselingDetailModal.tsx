...
const CounselingDetailModal: React.FC<CounselingDetailModalProps> = ({
  counseling,
  open,
  onClose,
}) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ border: "1px solid #A51C30", color: "#A51C30" }}>
        상세보기
      </DialogTitle>
      <DialogContent>
          ...
          <Table>
            <TableBody>
              <TableRow>
                ...
                <StyledTableCell>
                  {new Date(counseling.applicationDate).toLocaleString()}
                </StyledTableCell>
              </TableRow>
              <TableRow>
                <StyledTableCell>상담사</StyledTableCell>
                <StyledTableCell>
                  {counseling.counselor.user.userName}
                </StyledTableCell>
              </TableRow>
              <TableRow>
                <StyledTableCell>상담일시</StyledTableCell>
                <StyledTableCell>
                  {new Date(counseling.counselDate).toLocaleDateString()}{" "}
                  {getTimeFromAvailTime(counseling.schedule.availTime)}
                </StyledTableCell>
              </TableRow>
              <TableRow>
